// CNS Excel shadow runner — calibration sample via the shared helper.
...
maybe("CNS Excel shadow runner — calibration sample", () => {
  let report: SystemCalibrationReport;

  beforeAll(async () => {
    report = await runSystemShadowSample("cns");
  }, 900_000);
