// lower_limb Excel shadow runner — calibration + ADR-0001 threshold enforcement.
...
maybe("lower_limb Excel shadow runner — calibration sample", () => {
  let report: SystemCalibrationReport;

  beforeAll(async () => {
    report = await runSystemShadowSample("lower_limb");
  }, 900_000);
